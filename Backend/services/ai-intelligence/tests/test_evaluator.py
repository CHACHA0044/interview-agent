...
def test_evaluate_perfect_answer(mock_generate, mock_retrieve, sample_question):
    mock_retrieve.return_value = RetrievalResult(
        query="Architecture inversion of control decoupling",
        total_found=1,
        chunks=[RetrievedChunk(content="DI means decoupling...", score=0.9)]
    )
...
@patch("app.services.evaluator.retrieve")
@patch("app.services.evaluator.generate_structured_output")
def test_evaluate_empty_answer_fast_path(mock_generate, mock_retrieve, sample_question):
...
@patch("app.services.evaluator.retrieve")
@patch("app.services.evaluator.generate_structured_output")
def test_evaluate_llm_failure_fallback(mock_generate, mock_retrieve, sample_question):
