...
- Payload shapes follow the ai_api contract (camelCase, contract-shaped RetrievedChunk).
"""
...
def test_generate_interview_question_success(mock_generate, mock_retrieve, sample_strategy, sample_candidate):
    # Mock RAG returning a valid chunk
    mock_retrieve.return_value = RetrievalResult(
        query="FastAPI endpoints pydantic",
        total_found=1,
        chunks=[RetrievedChunk(day=5, title="FastAPI Basics", objectives=["FastAPI uses Pydantic for validation."], score=0.9)]
    )
...
@patch("app.services.question_generator.retrieve")
@patch("app.services.question_generator.generate_structured_output")
def test_generate_question_empty_retrieval(mock_generate, mock_retrieve, sample_strategy, sample_candidate):
...
@patch("app.services.question_generator.retrieve")
@patch("app.services.question_generator.generate_structured_output")
def test_generate_question_llm_failure_triggers_fallback(mock_generate, mock_retrieve, sample_strategy, sample_candidate):
    # Mock RAG returning chunks
    mock_retrieve.return_value = RetrievalResult(
        query="FastAPI", total_found=1, chunks=[RetrievedChunk(day=5, title="FastAPI", score=0.9)]
    )
